  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float reactivity = clamp(0.12 + shapeShift * 0.98 + uTrance * 0.2, 0.0, 1.0);
        ...
        q += organic * (
          0.01 + uMotionEnergy * 0.026 + uFlux * 0.02 + shapeShift * 0.024
        );
        ...
        float reactiveTravel = 0.58 + reactivity * 1.48;

        vec2 anchorInner = vec2(
          0.22 + frequencyShape * 0.065 + sin(drift * 0.71) * 0.018 * reactiveTravel,
          0.018 + sin(drift * 0.83 + 1.2) * 0.014 * reactiveTravel
        );
        vec2 anchorMiddle = vec2(
          0.51 + frequencyShape * 0.13 + sin(drift * 0.47 + 2.1) * 0.032 * reactiveTravel,
          0.075 + uSpectralMid * 0.065 + cos(drift * 0.64) * 0.022 * reactiveTravel
        );
        vec2 anchorOuter = vec2(
          0.84 + frequencyShape * 0.19 + cos(drift * 0.39 + 0.7) * 0.045 * reactiveTravel,
          0.13 + uSpectralHigh * 0.08 + sin(drift * 0.58 + 2.8) * 0.032 * reactiveTravel
        );
        ...
        float psychedelicPhase = tunnelPhase * 0.42 + tunnelQ.y * 0.34
          ...
          + organic.x * 0.28 + drift * 0.025 + shapeShift * 0.16;
        ...
        float psychedelicContour = 0.5 + 0.5 * cos(
          motifDistanceField * (48.0 + shapeShift * 28.0)
            - tunnelPhase * TAU * 1.7 + organic.y * 3.2 - drift * 0.38
        );
        color += fringe * psychedelicContour * motifAura
          * (0.025 + shapeShift * 0.085 + uFlux * 0.04);
